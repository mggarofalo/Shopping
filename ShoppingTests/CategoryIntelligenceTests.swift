...
struct CategoryIntelligenceTests {
    ...
    @Test("A recommendation only applies to the current normalized item name")
    func recommendationIdentityTracksItemName() {
        let request = CategoryIntelligenceRequest(
            itemName: "  Whole Milk ",
            candidates: CategoryIntelligenceFixtures.candidates
        )

        let categoryID = UUID()
        #expect(IntelligentCategoryPicker.requestMatchesCurrentDraft(
            request,
            itemName: "whole milk",
