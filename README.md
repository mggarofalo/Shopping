...
The local architecture decision and experimental persistence harness are recorded in [ADR 0001](docs/architecture/0001-managed-household-persistence.md). The app still displays the initial SwiftUI shell; the harness runs through tests. Live two-phone proof remains required in SHOPPING-30. Sharing is mandatory for release, but it is not yet proven. Enrollment work in SHOPPING-10 remains a gate for live CloudKit validation; local model, UI, and simulated-replica work can proceed before it.
