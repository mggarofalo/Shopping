...
Read the repository's `AGENTS.md` and [test strategy](../../../docs/test-strategy.md). Before adding or consolidating coverage, read the [proof ownership ledger](../../../docs/test-ownership.md). For CI, full-suite validation, or runtime work, also read [continuous integration](../../../docs/continuous-integration.md). These maintained documents own commands, environments, plan contents, and required gates. Follow the requested task's scope and authorization; this skill adds no permission to dispatch workflows or change policy.
...
- Before adding a UI scenario, name the user interaction it uniquely proves and the existing owners of related rules. Seed unrelated prerequisites with an isolated fixture; keep the UI steps that are themselves the contract. A fixture for promotion must seed a one-time occurrence, not a completed promotion. Check fixture identity, saved fields, lifecycle and relationships at the fast layer.
...
3. Make one focused change and run a targeted comparison with the same test inventory and environment. For an authorized redesign, record the original and replacement selections, map each removed assertion or setup step to its retained proof owner, and compare complete before/after scenarios; call out changed inventory. A changed toolchain, test selection, cold/warm build, or worker count must be stated. Do not infer a suite-wide saving from a small sample. Stop repeating an experiment when gains are unclear or correctness is unresolved.
...
Use focused benchmarks during exploration. When the final candidate requires remote `ShoppingFull`, follow the existing sequence: commit the candidate, pass [the local full runner](../../../.github/scripts/run-local-shopping-full.sh), push that exact unchanged commit, then use [the dispatch helper](../../../.github/scripts/dispatch-remote-shopping-full.sh). Edits after attestation invalidate that candidate's proof. Preserve the hosted exact-SHA preflight, coverage collection, coverage baselines, and intended behavioral proof. Test inventory may be consolidated when the task authorizes it and the ownership ledger accounts for each retired scenario. Audit plan selections against actual result identifiers so a stale name or filter cannot silently omit proof. Workflow-policy changes need their own authorization.

Use the maintained quick acceptance plan for routine PR feedback and the full plan for the broader required validation. Adding a test to quick acceptance requires a representative workflow reason and measured cost; related rule permutations belong in the fast layer. Keep abrupt termination, ordinary relaunch, accessibility and appearance boundaries distinct.
